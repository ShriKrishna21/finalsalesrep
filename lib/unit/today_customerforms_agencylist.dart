...
class todaycustomerformagencylist extends StatefulWidget {
  final User user;
  
  const todaycustomerformagencylist({super.key, required this.user});

  @override
  State<todaycustomerformagencylist> createState() => _todaycustomerformagencylistState();
}

class _todaycustomerformagencylistState extends State<todaycustomerformagencylist> {
  List<Record> records = [];
...
// 8566682
  ...
  List<String> getUniqueAgencyNames(List<Record> records) {
  final Set<String> uniqueNames = {};

  for (var record in records) {
    if (record.agency != null && record.agency!.isNotEmpty) {
      uniqueNames.add(record.agency!);
    }
  }

  return uniqueNames.toList()..sort(); // sorted list
}
  Map<String, int> getAgencyCounts(List<Record> records) {
  final Map<String, int> counts = {};

  for (var record in records) {
    final agency = record.agency ?? '';
...
Widget build(BuildContext context) {
  ...
  final uniqueAgencies = getUniqueAgencyNames(filteredRecords);
  final agencyCounts = getAgencyCounts(filteredRecords);

  return Scaffold(
    appBar: AppBar(
      title: Text("Agencies"  ' (${uniqueAgencies.length})'),
    ),
          ...
          : uniqueAgencies.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    final count = agencyCounts[agencyName] ?? 0;

                    return ListTile(
                      title: Row(
                        children: [
                          Expanded(child: Text(agencyName)),
                          Text(
                            '($count)',
                            style: TextStyle(
                      ...
                      onTap: () {
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => ToadayCustomerformsAgent(
                              user: widget.user,
